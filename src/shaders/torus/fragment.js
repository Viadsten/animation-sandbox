...
void main() {
    float pat = pnoise(vec3(vUv * uNoise , uTime * 0.4 )) * uDisplace ;
    float proximity = abs(vUv.x - (.5 + sin(uTime)/(12. * uSpread ) ));

    vec3 full = pat * vec3(.23 * uSpread  - proximity , 0., 1.);
    vec3 newPosition = vPosition + vNormal * full; 
    vec3 purpleColor = vec3(0.998, 0.2039, 0.8314) / vec3(0.1941, 0.2941, 0.051);
